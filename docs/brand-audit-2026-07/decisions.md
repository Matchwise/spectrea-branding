...
BA-10 | ratified-by-Darren | option (b): re-scope to the gradient (Cool Duet) family. Verified during implementation: no surface ever rendered a true full-spectrum lockup (LogoExploration:397 renders LogotypeGradient); the defect was pure naming drift. Implemented: style id/label -> gradient-white "Gradient / White" (markColorMode cool), asset renamed logo-lockup-gradient-white.svg, guide records it as form 1's dark rendering with resolution note; two-forms rule intact | by orchestrator
D2-ONRAMP | ratified-by-Darren | outcome-first hero, no whole-product term. Implemented as positioning.onRamp in brand.ts (posture + adopt/avoid/coined + coinRule), guide mirror in voice section, contract vocabulary.onRamp + agent-rules On-ramp line via generator; hero/social copy application routed to a follow-up cycle | by orchestrator
BA-24F | ratified-by-Darren | codified silence on origin. Implemented: originStance.status undecided -> decided-silence with dated rule, guide executive-voice mirror updated | by orchestrator
D4-VETO | ratified-by-Darren | keep everything: neverUse AI-hype additions, GitHub bio, icons.svg deletion, trustCopy masters all stand | by orchestrator
