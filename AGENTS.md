...
- **Releases mark selected versions, not every bump.** A GitHub release is the pin consumers adopt; `main` is always the newest canon. Cutting one, and what qualifies: `docs/releases.md`. Release notes are public — write them from the published artefacts, never from `internalCanon` fields.
