    ...
    >
      ...
      <Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Default card */}
          <div>
            <p className="text-xs font-semibold text-pewter uppercase tracking-wider mb-2">Default</p>
            <div className="border border-stone-200 rounded-xl p-5 bg-white">
              <p className="text-sm font-semibold text-ink" style={{ fontFamily: "'Albert Sans', sans-serif" }}>Card title</p>
              <p className="text-xs text-slate mt-1">A short supporting description. Keep it to one or two lines so the card stays scannable.</p>
          ...
          {/* Elevated card */}
          <div>
            <p className="text-xs font-semibold text-pewter uppercase tracking-wider mb-2">Elevated</p>
            <div className="rounded-xl p-5 bg-white shadow-md border border-stone-100 hover:shadow-lg transition-shadow">
              <p className="text-sm font-semibold text-ink" style={{ fontFamily: "'Albert Sans', sans-serif" }}>Quick Stats</p>
              <div className="grid grid-cols-2 gap-2 mt-3">
                <div>
                  <p className="text-lg font-semibold text-ink" style={{ fontFamily: "'Albert Sans', sans-serif" }}>2.8k</p>
                ...
                <div>
                  ...
                  <p className="text-xs text-slate">Growth</p>
                </div>
              </div>
            </div>
            <p className="text-xs text-slate mt-2">Shadow-md, hover lifts to shadow-lg.</p>
          </div>

          {/* Interactive card */}
          <div>
            <p className="text-xs font-semibold text-pewter uppercase tracking-wider mb-2">Interactive</p>
            <div className="border border-stone-200 rounded-xl p-5 bg-white hover:border-stone-300 hover:bg-cloud active:bg-cloud transition-all cursor-pointer group">
              <p className="text-sm font-semibold text-ink group-hover:text-brand transition-colors" style={{ fontFamily: "'Albert Sans', sans-serif" }}>Section title</p>
              <p className="text-xs text-slate mt-1">Short supporting line. Two lines maximum.</p>
            ...
            <p className="text-xs text-slate mt-2">Hover: border darkens, title shifts to Cobalt, arrow nudges.</p>
          </div>
        </div>
      </Section>
      ...
      <Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="border border-stone-200 rounded-xl p-5">
            <p className="text-xs font-semibold text-pewter uppercase tracking-wider mb-3">Stat Card</p>
            <div className="rounded-lg border border-stone-200 p-4 hover:border-stone-300 transition-colors">
              <p className="text-xs text-slate">Active Items</p>
              <p className="text-2xl font-semibold text-ink mt-0.5" style={{ fontFamily: "'Albert Sans', sans-serif" }}>2,847</p>
              <p className="text-xs font-medium mt-1" style={{ color: '#00B6A0' }}>+12% from last month</p>
            </div>
            <p className="text-xs text-slate mt-2">Label → Value → Trend. Always in this order.</p>
          </div>

          {/* Detail card */}
          <div className="border border-stone-200 rounded-xl p-5">
            <p className="text-xs font-semibold text-pewter uppercase tracking-wider mb-3">Detail Card</p>
            <div className="rounded-lg border border-stone-200 p-4 hover:border-stone-300 hover:bg-cloud/50 transition-all cursor-pointer">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-semibold text-ink" style={{ fontFamily: "'Albert Sans', sans-serif" }}>Item title</p>
                <span className="text-xs font-mono px-1.5 py-0.5 rounded" style={{ backgroundColor: '#4271DF10', color: '#4271DF' }}>Type</span>
              </div>
              <p className="text-xs text-slate">A short description of what this item represents and why it might matter to the reader.</p>
              <div className="flex items-center gap-3 mt-3 pt-3 border-t border-stone-100">
                <span className="text-xs text-slate font-mono">94% confidence</span>
                <span className="text-xs text-slate">12 links</span>
                <span className="text-xs text-slate">Updated 2h ago</span>
              </div>
            </div>
            <p className="text-xs text-slate mt-2">Title + type → Description → Metadata footer.</p>
          </div>
        </div>
      </Section>
      ...
      <Section>
        ...
        {visibleAlerts.length > 0 ? (
          <div className="space-y-3">
            <p className="text-[10px] font-semibold text-pewter uppercase tracking-wider">Interactive — click to dismiss</p>
            {visibleAlerts.map(a => (
              <div
                key={a.title}
      ...
      <Section>
        ...
        <div className="border border-stone-200 rounded-xl p-6 bg-white">
          <div className="space-y-5">
            ...
            <div>
              <p className="text-xs font-semibold text-pewter uppercase tracking-wider mb-2">Spectrum Tags — click to remove</p>
              ...
              <p className="text-xs text-slate mt-2">Rounded-full, 8% background opacity, 15% border opacity.</p>
            </div>

            {/* Neutral tags */}
            <div>
              <p className="text-xs font-semibold text-pewter uppercase tracking-wider mb-2">Neutral Tags</p>
              <div className="flex gap-2 flex-wrap">
                {['Category', 'Type', 'v2.1', 'Draft'].map(label => (
                  <span key={label} className="text-xs px-2.5 py-1 rounded-full bg-cloud text-iron border border-stone-200">{label}</span>
                ))}
              </div>
              <p className="text-xs text-slate mt-2">For non-semantic metadata: types, versions, statuses without urgency.</p>
            </div>

            {/* Count badges */}
            <div>
              <p className="text-xs font-semibold text-pewter uppercase tracking-wider mb-2">Count Badges</p>
              <div className="flex items-center gap-4 flex-wrap">
                <span className="inline-flex items-center gap-1.5 text-xs text-iron">
              ...
              <p className="text-xs text-slate mt-2">Solid fill for high urgency (notifications, errors). Tinted fill for informational counts.</p>
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Skeleton */}
          <div className="border border-stone-200 rounded-xl p-5">
            <p className="text-xs font-semibold text-pewter uppercase tracking-wider mb-3">Skeleton Loading</p>
            <div className="rounded-lg border border-stone-200 p-4 space-y-3">
              ...
              <div className="space-y-1.5">
                <div className="h-2 w-full bg-cloud rounded animate-pulse" />
                <div className="h-2 w-3/4 bg-cloud rounded animate-pulse" />
              </div>
              <div className="flex gap-2 pt-2 border-t border-stone-100">
                <div className="h-2 w-12 bg-cloud rounded animate-pulse" />
                <div className="h-2 w-16 bg-cloud rounded animate-pulse" />
                <div className="h-2 w-14 bg-cloud rounded animate-pulse" />
              </div>
            </div>
            <p className="text-xs text-slate mt-2">Match the card's content structure. Animate with pulse. Use Stone 100-200 fills.</p>
          </div>

          {/* Empty state */}
          <div className="border border-stone-200 rounded-xl p-5">
            <p className="text-xs font-semibold text-pewter uppercase tracking-wider mb-3">Empty State</p>
            <div className="rounded-lg border border-dashed border-stone-300 p-6 text-center">
              <div className="w-10 h-10 rounded-full bg-cloud flex items-center justify-center mx-auto mb-3">
                <span className="text-pewter text-lg">+</span>
              </div>
              <p className="text-sm font-medium text-iron">Nothing here yet</p>
              <p className="text-xs text-slate mt-1">Add your first item to get started.</p>
            ...
            <p className="text-xs text-slate mt-2">Dashed border, icon, explanation, and a CTA. Never leave empty space unexplained.</p>
          </div>
        </div>
      </Section>
