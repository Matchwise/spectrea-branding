...
export const internalCanon = {
  ...
  fields: [
    ...
    // Ratified 2026-08-18. The ledger is the decision RECORD — 40+ dated
    // entries reasoning about positioning, claims, competitors and process.
    // Publishing it hands a reader the deliberation, not the identity, and it
    // was the last long-form strategy text on a crawler-readable surface. The
    // decisions stay traceable in this repo\'s public git history; the record
    // itself now travels with the other internal-tier fields.
    'ratificationLedger',
  ],
} as const
...
export const ratificationLedger = [
  ...
  { date: '2026-08-18', decision: 'The ratification ledger itself becomes internal-tier (v2.15.0, Darren-ratified). It rendered in two public places — the Governance page and brand-contract.json — and it is the one remaining long-form strategy text on a crawler-readable surface: 40+ dated entries reasoning about positioning, claims, named competitors, and how decisions get made. That is deliberation, not identity, and the guide is an identity system. Registered in internalCanon, so it now renders only to the internal artefacts, exactly like the trust masters. What stays public: the FACT of ratified governance (the change process, the owner, the rule that a decision recorded only in a consumer repo is not canon until it lands in the ledger) and the decisions themselves through this repo\'s public git history — the tier reduces exposure, it does not claim secrecy. The probes for this field are scoped to brand surfaces (HISTORICAL_DOC_EXEMPT), because docs/ is the historical record and quotes ledger entries by design; a served artefact carrying them is still a build defect. Consumers: spectrea vendors brand.ts and keeps the ledger; spectrea-web reads it through the internal hand-off from now on.' },
] as const
